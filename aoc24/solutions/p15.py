...
def load_move(c: str) -> Offset | None:
    return {
        "v": Offset(1, 0),
        "^": Offset(-1, 0),
        "<": Offset(0, -1),
...
def double_grid_size(
    robot: Position, obstacles: set[Position], walls: set[Position]
) -> tuple[Position, set[Position], set[Position]]:
    robot = Position(robot.i, robot.j * 2)
    walls = {Position(w.i, w.j * 2) for w in walls} | {
        Position(w.i, w.j * 2 + 1) for w in walls
    }
    obstacles = {Position(o.i, o.j * 2) for o in obstacles}
    return robot, obstacles, walls


def find_large_obstacles_to_move(
    start: Position, move: Offset, obstacles: set[Position], walls: set[Position]
) -> set[Position]:
    ...
    while to_explore:
        ...
        if move.i == 0:
            # Horizontal move
            possible_obstacle = (
                position if move.j == 1 else move_position(position, move)
            )
            if possible_obstacle in obstacles:
                to_explore.add(move_position(position, move * 2))
                obstacles_to_move.add(possible_obstacle)
        else:
            # Vertical move. Check case where both left and right sides of obstacle are above/below position
            # Explore above both left and right sides
            for offset in [Offset(0, 0), Offset(0, -1)]:
                ...
                if possible_obstacle in obstacles:
                    ...
                    to_explore.add(
                        move_position(possible_obstacle, move + Offset(0, 1))
                    )
...
def p15b(f: TextIO) -> int:
    ...
    for i in f.read().strip():
        ...
        robot_new = move_position(robot, move)
        try:
            obstacles_to_move = find_large_obstacles_to_move(
                robot_new, move, obstacles, walls
            )
